...
for (c_label in unique(dmat_clara2_df$label)){
  ...
  for (c_sampling in unique(x$sampling)){
    ...
    data.frame(Data = paste(c_label, ' + ', c_sampling, sep = ''),
               formula = 'Avg. Avg. HD ~ Within to Between Dist. Ratio',
               intercept_est = smod$coefficients[1, 1],
               intercept_p.value = smod$coefficients[1,4],
               slope_est = smod$coefficients[2, 1],
               slope_p.value = smod$coefficients[2,4],
               adj.R.sq = smod$adj.r.squared,
               stringsAsFactors = FALSE)
    )

#    cat(paste('\n\nModel details for label ', c_label, 
#              ' with fitness processing ', c_sampling,
#              '\n\n', sep = ''))
#    print(summary())
#    cat('\n\n')
  }
