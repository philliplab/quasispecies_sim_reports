The following tables show some summary metrics for `r length(unique(all_sim_results$sim_n))` simulations.
...
dd.f <- NULL
for (i in unique(all_sim_results$sim_n)){
  dmat <- stringdistmatrix(last_gens$the_seq[last_gens$sim_n == i], method = 'hamming')
  avg_hd <- mean(dmat)
...
```{r, results = 'asis'}
all_deciles <- data.frame(matrix(0, ncol = 12, nrow = max(all_sim_results$sim_n)))
names(all_deciles) <- c('Sim', paste((0:10)*10, "%", sep = ''))

for (i in 1:max(all_sim_results$sim_n)){
  deciles <-
    quantile(dd.f$dists[dd.f$sim_n == i], (0:10)/10)
...
```{r, results = 'asis'}
g_sum_tab <- genealogy_summary_table(all_sim_results[all_sim_results$sim_n == 1, -1])
