...
The average pairwise HD is **`r round(avg_hd, 2)`** and normalized by the length of the sequence it is **`r round(avg_hd/min(nchar(last_gen$the_seq)), 3)`**. The distribution is shown in the figure below and the deciles are listed in a table below the figure. Note that the bandwidth has to be tweaked because mutations are rare events thus the distribution is fairly discrete.
